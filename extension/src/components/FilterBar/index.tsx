...
export const FilterBar: React.FC<FilterBarProps> = ({ filters, onFilterChange, isOpen, onToggle }) => {
  ...
  return (
    <div className="ig-border-b ig-border-zinc-800">
      {/* Filter Toggle Button */}
      <button
        onClick={onToggle}
        className={`ig-w-full ig-flex ig-items-center ig-justify-between ig-px-4 ig-py-2 ig-text-sm ig-transition-colors ${
          hasActiveFilters ? "ig-bg-blue-900/30 ig-text-blue-400" : "ig-bg-zinc-900 ig-text-zinc-400 hover:ig-bg-zinc-800"
        }`}>
        <div className="ig-flex ig-items-center ig-gap-2">
          <Filter className="ig-w-4 ig-h-4" />
          <span className="ig-font-medium">Filters</span>
          {hasActiveFilters && <span className="ig-bg-blue-500 ig-text-white ig-text-xs ig-px-2 ig-py-0.5 ig-rounded-full">Active</span>}
        </div>
        <svg
      ...
      {/* Filter Options */}
      {isOpen && (
        <div className="ig-p-4 ig-bg-zinc-900 ig-space-y-4 ig-animate-fade-in">
          {/* State Filter */}
          <div>
            <label className="ig-flex ig-items-center ig-gap-2 ig-text-xs ig-font-medium ig-text-zinc-400 ig-mb-2">
            ...
            <select
              ...
              className="ig-w-full ig-px-3 ig-py-2 ig-text-sm ig-bg-zinc-800 ig-border ig-border-zinc-700 ig-rounded-lg ig-text-zinc-100 focus:ig-ring-2 focus:ig-ring-blue-500 focus:ig-border-transparent ig-outline-none">
          ...
          {hasActiveFilters && (
            <button
              ...
              className="ig-w-full ig-py-2 ig-text-sm ig-text-blue-400 ig-font-medium hover:ig-bg-zinc-800 ig-rounded-lg ig-transition-colors">
              Clear All Filters
            </button>
          )}
        </div>
